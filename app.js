...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const tabsHeader = document.querySelector(".kanban-tabs");
  const columns = document.querySelectorAll(".kanban-col");

  if (tabsHeader && columns.length > 0) {
    tabsHeader.addEventListener("click", (e) => {
      const clickedTab = e.target.closest(".tab-item");
      if (!clickedTab) return;

      const allTabs = [...tabsHeader.querySelectorAll(".tab-item")];
      const clickedIndex = allTabs.indexOf(clickedTab);

      allTabs.forEach((tab) => tab.classList.remove("active"));
      clickedTab.classList.add("active");

      columns.forEach((col) => col.classList.remove("on"));
      columns[clickedIndex].classList.add("on");
    });
  }
